...
class CornDatasetCrop(Dataset):
    ''' returns images, labels(GT) - general dataset for class-segmentation'''
    def __init__(self, root_dir='./', root_crop_dir='./', type_="train", size=None, transform=None):
        self.root_dir = root_dir
        self.root_crop_dir = root_crop_dir
        self.type = type_
        ...
        if 'train' in self.type:
            # get image, foreground and instance list
            image_crop_list = glob.glob(os.path.join(self.root_crop_dir, self.type, '*_sharp.png'))
            image_crop_list.sort()
            self.image_crop_list = image_crop_list
            print("# image(crop) files: ", len(image_crop_list))

            label_crop_list = glob.glob(os.path.join(self.root_crop_dir, self.type, '*_overall.png'))
            label_crop_list.sort()
            self.label_crop_list = label_crop_list
            print("# label(crop) files: ", len(label_crop_list))
    ...
    def __getitem__(self, index):
        ...
        # load image and foreground (original)
        ...
        if 'train' in self.type:
            # load image and foreground (cropped)
            image_crop = Image.open(self.image_crop_list[index]).convert('RGB')
            image_crop = image_crop.resize((h, w), resample=Image.Resampling.BILINEAR)

            width, height = image_crop.size

            sample['image_crop'] = image_crop.copy()
            sample['im_name_crop'] = self.image_crop_list[index]

            label_map_crop = skimage.io.imread(self.label_crop_list[index])  # := instance map
            label_map_crop = cv2.resize(label_map_crop, (h, w), interpolation=cv2.INTER_NEAREST)
            class_ids = np.unique(label_map_crop)[1:]  # no background

            label_crop = np.zeros((height, width), dtype=np.uint8)

            for idx in class_ids:
                if idx == 60:
                    mask_plant = (label_map_crop == idx)
                    label_crop[mask_plant] = 1

                elif idx == 120:
                    mask_disease = (label_map_crop == idx)
                    label_crop[mask_disease] = 2
        ...
        if 'train' in self.type:
            ...
            if random.random() > 0.5:
                # FLIP_TOP_BOTTOM
                sample['image_crop'] = sample['image_crop'].transpose(Image.FLIP_LEFT_RIGHT)
                label_crop = np.fliplr(label_crop)
            ...
            if random.random() > 0.5:
                # FLIP_LEFT_RIGHT
                sample['image_crop'] = sample['image_crop'].transpose(Image.FLIP_TOP_BOTTOM)
            ...
            # 2.crop
            if random.random() > 0.5:
                img_np = np.array(sample['image_crop'])
                img_np = cv2.rotate(img_np, cv2.ROTATE_90_CLOCKWISE)
                sample['image_crop'] = Image.fromarray(img_np)
            ...
            if random.random() > 0.5:
                ...
                img_np = cv2.rotate(img_np, cv2.ROTATE_90_COUNTERCLOCKWISE)
                sample['image_crop'] = Image.fromarray(img_np)
                label_crop = cv2.rotate(label_crop, cv2.ROTATE_90_COUNTERCLOCKWISE)
            ...
            # 2.crop
            if random.random() > 0.5:
                sample['image_crop'] = random_background(img_pil=sample['image_crop'])
            ...
            # 2.crop
            if random.random() > 0.5:
                sample['image_crop'] = random_gamma(img_pil=sample['image_crop'])
            ...
            # 2.crop
            if random.random() > 0.7:
                sample['image_crop'] = grayscaler(sample['image_crop'])
        ...
        if 'train' in self.type:
            ...
            # 2.crop
            label_crop = Image.fromarray(np.uint8(label_crop))
            sample['label_crop'] = label_crop
